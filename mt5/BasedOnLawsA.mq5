...
input double InpFixedStopPts  = 1.00;    // 2026-08-25, HIS CALL: "activate this winrate
                                        ...
                                        // on trial only.
input double InpTpPoints      = 0.14;   // THE WIN-RATE EXPERIMENT. At 0.01 lots this
...
int OnInit() {
   ...
   if (MQLInfoInteger(MQL_TESTER))
      PrintFormat("[LAW] chart FROZEN for this run — %d OANDA volume rows, %d OANDA bars",
                  g_ov_n, g_ob_n);
   PrintFormat("[LAWA] BasedOnLawsA v1.03 (tp 0.14 / sl 1.00) — LAWS.md only. buy%s · NY %s · M5+M15 %s · "
...
void OnTick() {
   ...
   bool ok = (t > 0) ? trade.Buy(InpLots, _Symbol, 0, sl, tp, "zlawA_buy")
                     : trade.Sell(InpLots, _Symbol, 0, sl, tp, "zlawA_sell");
   if (ok) c_fired++;
   if (ok) {
