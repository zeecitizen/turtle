//+------------------------------------------------------------------+
//| ZeeUHVSec.mq5 — does the fractal setup EARN at 1-second scale?    |
//|                                                                  |
...
// magic 88103 — its own. Never 88094 (live) or 88095-88102.
#property strict

input double InpLots        = 0.02;  // small: this fires ~100x more than the M1 engine
input int    InpMagicNumber = 88103;
input double InpStopPts     = 5.0;   // the live EA's stop
input double InpTargetPts   = 1.0;   // the live EA's target
input int    InpHoldSec     = 300;   // 300 s = the live EA's 5-minute hold
input int    InpMaxOpenSec  = 10;    // a setup every ~122 s against a 300 s hold
...
//--- one ticket per setup, so the number reported IS the per-setup edge
int OpenCountSec() {
   int n = 0;
...
void Fire(int side) {
   if (OpenCountSec() >= InpMaxOpenSec) return;
   MqlTick tk;
   if (!SymbolInfoTick(_Symbol, tk) || tk.ask <= 0 || tk.bid <= 0) return;
   ...
   if (side > 0) trade.Buy (InpLots, _Symbol, 0, sl, tp, "sec");
   else          trade.Sell(InpLots, _Symbol, 0, sl, tp, "sec");
}

//--- the hold, in SECONDS because that is the scale we are testing
void AgeOutSec() {
   for (int i = PositionsTotal() - 1; i >= 0; i--) {
      ulong tk = PositionGetTicket(i);
      ...
      if (TimeCurrent() - opened >= InpHoldSec) trade.PositionClose(tk);
   }
}
