//+------------------------------------------------------------------+
//|  ZeeUHV_D07.mq5 — THE UNTOUCHED DIAMOND, resurrected         |
...
input int    InpMagicNumber = 88207;  // InpMagicNumber — 88094 = ZeeUHV, tester only
...
input double InpLawStructStop = 0.60;   // >0: stop this many pips under the retracement low
...
void LoadOandaVol() {
   ...
   if (h == INVALID_HANDLE) {
      Print("[D07] OANDA volume requested but oanda_vol.csv not found — using broker volume");
      return;
   }
   ...
   PrintFormat("[D07] OANDA volume table loaded: %d minutes (newest %s)",
...
int OnInit() {
   ...
   PrintFormat("[D07] ZeeUHV v1.10 — HIS rules from 146 labels. SL %.1f / TP %.1f · magic %d"
...
void OnDeinit(const int r) { PrintFormat("[D07] deinit reason=%d", r); }
...
void TryFire() {
   ...
   if (!WindowContinuous(InpTrendLook + 5)) {
      if (InpVerbose) Print("[D07] [SKIP] gap in lookback");
      return;
   }
...
//  both sides and take whichever completes a lawful setup.
   ...
   else { if (InpVerbose) Print("[D07] [SKIP] ranging — his setup needs a trend"); return; }
   ...
   if (side == 0) {
      if (InpVerbose && t != 0) Print("[D07] [SKIP] no lawful setup on the allowed side");
      return;
   }
   t = side;
   ...
   for (int q = 0; q < tickets; q++) {
      ...
      string tag = StringFormat("zd07_%s_D%d", (t > 0 ? "buy" : "sell"), dia);
   ...
   if (placed > 0) {
      ...
      PrintFormat("[D07] %s @%.2f — %d diamond(s) -> %d ticket(s), %.2f lots total · "
   ...
   for (int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if (TimeCurrent() - opened >= InpMaxHoldMin * 60)
         if (trade.PositionClose(t) && InpVerbose)
            PrintFormat("[D07] aged out after %dm", InpMaxHoldMin);
   }
}
