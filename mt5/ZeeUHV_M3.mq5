...
//|  FIRST net-positive six-fortnight total:                         |
//|                                                                  |
//|     LIVE +0.96 · Mar −16.32 · Apr −7.28 · May −15.24             |
//|     Jun +80.12 · Jul −18.92  == +23.32, worst period −19         |
//|     (vs the M1 shop's −228.88 with a −384 worst period)          |
//|                                                                  |
//|  ATTACH TO: XAUUSD *** M3 *** chart. On any other timeframe the  |
//|  receipts above do not apply (M1/M2/M4/M5 all measured worse).   |
//|  Magic 88134 — its own ledger, alongside the M1 shop, never      |
//|  replacing it. Same laws: Law 9 origins, rank-6 auditions,       |
//|  Law 10c loud-band quarter-sizing, hold 3 min = ONE M3 CANDLE.   |
//|                                                                  |
//|  Identical source lineage to ZeeUHV v1.36 — only identity        |
//|  (name, magic, log tag, comment prefix) differs, so every        |
...
input int    InpMagicNumber = 88134;  // InpMagicNumber — 88094 = ZeeUHV, tester only
...
int OnInit() {
   ...
   PrintFormat("[ZB] PRICE via SymbolInfoTick + tick-history cross-check %s — refuse if "
...
void OnDeinit(const int r) { PrintFormat("[ZB] deinit reason=%d", r); }
...
bool CurrentTick(MqlTick &out) {
   if (!SymbolInfoTick(_Symbol, out)) {
      PrintFormat("[ZB] [BLOCKED] SymbolInfoTick failed (%d) — no price, no trade",
                  GetLastError());
      return false;
   }
   if (out.ask <= 0 || out.bid <= 0) {
      PrintFormat("[ZB] [BLOCKED] nonsense quote bid=%.2f ask=%.2f", out.bid, out.ask);
      return false;
   }
   if (!SymbolIsSynchronized(_Symbol)) {
      Print("[ZB] [BLOCKED] symbol is NOT synchronized with the server — refusing to trade");
      return false;
   }
   if (InpMaxQuoteAgeSec > 0) {
      long age = (long)(TimeCurrent() - out.time);
      if (age > InpMaxQuoteAgeSec) {
         PrintFormat("[ZB] [BLOCKED] the tick is %d s old (limit %d) — this is the "
   ...
   if (InpMaxQuoteDrift > 0) {
      ...
      if (CopyTicks(_Symbol, h, COPY_TICKS_INFO, 0, 1) == 1 && h[0].ask > 0) {
         if (MathAbs(h[0].ask - out.ask) > InpMaxQuoteDrift) {
            PrintFormat("[ZB] [BLOCKED] cache says %.2f, tick history says %.2f — %.2f "
...
void TryFire() {
   ...
   if (!WindowContinuous(InpTrendLook + 5)) {
      z_gap++;
      if (InpVerbose) Print("[ZB] [SKIP] gap in lookback");
      return;
   }
...
//  both sides and take whichever completes a lawful setup.
   ...
   else { z_ranging++; h_rang[z_hr]++; if (InpVerbose) Print("[ZB] [SKIP] ranging — his setup needs a trend"); return; }
   ...
   if (side == 0) {
      if (InpVerbose && t != 0) Print("[ZB] [SKIP] no lawful setup on the allowed side");
      return;
   }
   t = side;
   ...
   if (InpMaxQuoteAgeSec > 0) {
      ...
      if (qt > 0 && age > InpMaxQuoteAgeSec) {
         PrintFormat("[ZB] [BLOCKED] quote is %d s old (limit %d) — refusing to trade on it",
   ...
   if (InpMaxQuoteDrift > 0) {
      ...
      if (c0 > 0 && MathAbs(px - c0) > InpMaxQuoteDrift) {
         PrintFormat("[ZB] [BLOCKED] quote %.2f vs tape %.2f — %.2f pts apart (limit %.2f). "
   ...
   if (InpMaxSpreadPts > 0 && (ask - bid) > InpMaxSpreadPts) {
      PrintFormat("[ZB] [BLOCKED] spread %.2f over limit %.2f", ask - bid, InpMaxSpreadPts);
      return;
   }
   if (InpMaxPullback > 0) {
   ...
   if (InpProbeSec > 0) {
      ...
      bool pok = (t > 0) ? trade.Buy (InpProbeLots, _Symbol, 0, 0.0, 0.0, "zb_probe")
                         : trade.Sell(InpProbeLots, _Symbol, 0, 0.0, 0.0, "zb_probe");
      if (pok) {
         ...
         PrintFormat("[ZB] [PROBE] scout %s @%.2f — %ds to prove %.2f pts",
   ...
   for (int q = 0; q < tickets; q++) {
      ...
      string tag = StringFormat("zb_%s_D%d_m%d", (t > 0 ? "buy" : "sell"), dia, g_lawmask);
      ...
      if (pt > 0 && PositionSelectByTicket(pt)) {
         ...
         if (MathAbs(fill - px) > _Point) {
            ...
            if (!trade.PositionModify(pt, nsl, ntp))
               PrintFormat("[ZB] !! could not re-anchor #%I64u (%d) — still on QUOTE levels",
                           pt, trade.ResultRetcode());
            else if (MathAbs(fill - px) >= 1.0)
               PrintFormat("[ZB] !! fill %.2f vs quote %.2f (%.2f pts) — levels re-anchored "
   ...
   if (placed > 0) {
      g_last_fire = TimeCurrent();
      PrintFormat("[ZB] %s @%.2f — %d diamond(s) -> %d ticket(s), %.2f lots total · "
   ...
   for (int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if (TimeCurrent() - opened >= InpMaxHoldMin * 60)
         if (trade.PositionClose(t) && InpVerbose)
            PrintFormat("[ZB] aged out after %dm", InpMaxHoldMin);
   }
}
...
void FireBasket(int side, int dia, int mask, bool loud) {
   ...
   string tag = StringFormat("zb_%s_D%d_m%d", (side > 0 ? "buy" : "sell"), dia, mask);
   ...
   if (placed > 0) {
      g_last_fire = TimeCurrent();
      PrintFormat("[ZB] BURST %s @%.2f — %d diamond(s) -> %d ticket(s), pre-tested ground",
...
void ProbeManage() {
   ...
   if (moved >= InpProbeMinPts) {
      PrintFormat("[ZB] [PROBE] conviction %.2f pts -> BURST", moved);
      FireBasket(g_probe_side, g_probe_dia, g_probe_mask, g_probe_loud);  // pre-tested environment
   } else if (InpVerbose)
      PrintFormat("[ZB] [PROBE] only %.2f pts -> retreat, no basket", moved);
}
