//+------------------------------------------------------------------+
//|  BasedOnLawsNoGate.mq5 — the CONTROL: same laws, no trend gate                |
//|                                                                  |
...
int OnInit() {
   ...
   if (MQLInfoInteger(MQL_TESTER))
      PrintFormat("[LAW] chart FROZEN for this run — %d OANDA volume rows, %d OANDA bars",
                  g_ov_n, g_ob_n);
   PrintFormat("[LAW] BasedOnLawsNoGate v1.29 — LAWS.md only. buy%s · NY %s · M5+M15 %s · "
...
void OnTick() {
   ...
   // CONTROL ARM (2026-08-24): switching InpRequireTrend off used to change nothing,
   // because this line still demanded trend==+1 — the "no gate" test silently re-ran
   // the gated config and returned identical numbers. With no gate, buy-side simply
   // means "we only buy", not "we only buy in a proven uptrend".
   if (!InpRequireTrend) t = +1;
