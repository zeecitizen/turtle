//+------------------------------------------------------------------+
//|  ZeeUHV_D06.mq5 — THE UNTOUCHED DIAMOND, resurrected         |
...
input int    InpMagicNumber = 88206;  // InpMagicNumber — 88094 = ZeeUHV, tester only
...
input bool   InpLawNoPeak     = true; // drop the neighbour-peak test (he revoked it 2026-08-04)
...
void LoadOandaVol() {
   ...
   if (h == INVALID_HANDLE) {
      Print("[D06] OANDA volume requested but oanda_vol.csv not found — using broker volume");
      return;
   }
   ...
   PrintFormat("[D06] OANDA volume table loaded: %d minutes (newest %s)",
...
int OnInit() {
   ...
   PrintFormat("[D06] ZeeUHV v1.10 — HIS rules from 146 labels. SL %.1f / TP %.1f · magic %d"
...
void OnDeinit(const int r) { PrintFormat("[D06] deinit reason=%d", r); }
...
void TryFire() {
   ...
   if (!WindowContinuous(InpTrendLook + 5)) {
      if (InpVerbose) Print("[D06] [SKIP] gap in lookback");
      return;
   }
...
//  both sides and take whichever completes a lawful setup.
   ...
   else { if (InpVerbose) Print("[D06] [SKIP] ranging — his setup needs a trend"); return; }
   ...
   if (side == 0) {
      if (InpVerbose && t != 0) Print("[D06] [SKIP] no lawful setup on the allowed side");
      return;
   }
   t = side;
   ...
   for (int q = 0; q < tickets; q++) {
      ...
      string tag = StringFormat("zd06_%s_D%d", (t > 0 ? "buy" : "sell"), dia);
   ...
   if (placed > 0) {
      ...
      PrintFormat("[D06] %s @%.2f — %d diamond(s) -> %d ticket(s), %.2f lots total · "
   ...
   for (int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if (TimeCurrent() - opened >= InpMaxHoldMin * 60)
         if (trade.PositionClose(t) && InpVerbose)
            PrintFormat("[D06] aged out after %dm", InpMaxHoldMin);
   }
}
