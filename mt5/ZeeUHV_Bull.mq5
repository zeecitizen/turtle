//+------------------------------------------------------------------+
//|  ZeeUHV_Bull.mq5 — THE BULL: buys only, the uptrend is ASSUMED   |
//|                                                                  |
//|  Zee, 2026-08-20: "instead of taking both buys and sells, we     |
//|  take buys only. we treat every red retracement after a prior    |
//|  green as a retracement in an uptrend, because gold tends to     |
//|  keep going up. we consider the downs as pullbacks (among those  |
//|  pullbacks we find the highest volume red candle) then we mark   |
//|  its upwards break only. what i wanna make sure is that our EA   |
//|  is not skipping stuff."                                         |
//|                                                                  |
//|  DESIGN: ZeeUHV v1.49 lineage with two identity changes:         |
//|    InpBuysOnly = true  — the sell side does not exist here       |
//|    InpRequireTrend = false — no trend gate: the secular gold     |
//|      uptrend IS the trend call; every red pullback qualifies     |
//|  All other laws intact (origin, Law 9, rank 6, 10c, clock,       |
//|  pulse). DiamondMode off at birth — the court decides her        |
//|  geometry. Magic 88144 · log tag [BULL] · tickets zbull_*.       |
//+------------------------------------------------------------------+
...
input int    InpMagicNumber = 88144;  // InpMagicNumber — 88094 = ZeeUHV, tester only
...
input bool   InpRequireTrend = false;  // InpRequireTrend — false lets RANGING tape trade too (the 40% gate)
...
input bool   InpDiamondMode  = false; // LIVE 2026-08-19 (Zee: "Ship the Diamond Season machine
...
input bool   InpBuysOnly     = true; // THE BULL: sells do not exist; gold's secular uptrend is assumed
...
int OnInit() {
   ...
   PrintFormat("[BULL] ZeeUHV_Bull v1.00 — BUYS ONLY, uptrend assumed — HIS rules from 146 labels. SL %.1f / TP %.1f · magic %d"
   ...
   PrintFormat("[BULL] PRICE via SymbolInfoTick + tick-history cross-check %s — refuse if "
...
void OnDeinit(const int r) { PrintFormat("[BULL] deinit reason=%d", r); }
...
bool CurrentTick(MqlTick &out) {
   if (!SymbolInfoTick(_Symbol, out)) {
      PrintFormat("[BULL] [BLOCKED] SymbolInfoTick failed (%d) — no price, no trade",
                  GetLastError());
      return false;
   }
   if (out.ask <= 0 || out.bid <= 0) {
      PrintFormat("[BULL] [BLOCKED] nonsense quote bid=%.2f ask=%.2f", out.bid, out.ask);
      return false;
   }
   if (!SymbolIsSynchronized(_Symbol)) {
      Print("[BULL] [BLOCKED] symbol is NOT synchronized with the server — refusing to trade");
      return false;
   }
   if (InpMaxQuoteAgeSec > 0) {
      long age = (long)(TimeCurrent() - out.time);
      if (age > InpMaxQuoteAgeSec) {
         PrintFormat("[BULL] [BLOCKED] the tick is %d s old (limit %d) — this is the "
   ...
   if (InpMaxQuoteDrift > 0) {
      ...
      if (CopyTicks(_Symbol, h, COPY_TICKS_INFO, 0, 1) == 1 && h[0].ask > 0) {
         if (MathAbs(h[0].ask - out.ask) > InpMaxQuoteDrift) {
            PrintFormat("[BULL] [BLOCKED] cache says %.2f, tick history says %.2f — %.2f "
...
void TryFire() {
   ...
   if (!WindowContinuous(InpTrendLook + 5)) {
      z_gap++;
      if (InpVerbose) Print("[BULL] [SKIP] gap in lookback");
      return;
   }
...
//  both sides and take whichever completes a lawful setup.
   ...
   // THE BULL IDENTITY: the uptrend is assumed — buys only, always eligible
   if (InpBuysOnly) { sides[0] = +1; nsides = 1; }
   else { z_ranging++; h_rang[z_hr]++; if (InpVerbose) Print("[BULL] [SKIP] ranging — his setup needs a trend"); return; }
   ...
   if (side == 0) {
      if (InpVerbose && t != 0) Print("[BULL] [SKIP] no lawful setup on the allowed side");
      return;
   }
   t = side;
   ...
   if (InpMaxQuoteAgeSec > 0) {
      ...
      if (qt > 0 && age > InpMaxQuoteAgeSec) {
         PrintFormat("[BULL] [BLOCKED] quote is %d s old (limit %d) — refusing to trade on it",
   ...
   if (InpMaxQuoteDrift > 0) {
      ...
      if (c0 > 0 && MathAbs(px - c0) > InpMaxQuoteDrift) {
         PrintFormat("[BULL] [BLOCKED] quote %.2f vs tape %.2f — %.2f pts apart (limit %.2f). "
   ...
   if (InpMaxSpreadPts > 0 && (ask - bid) > InpMaxSpreadPts) {
      PrintFormat("[BULL] [BLOCKED] spread %.2f over limit %.2f", ask - bid, InpMaxSpreadPts);
      return;
   }
   if (InpMaxPullback > 0) {
   ...
   if (InpProbeSec > 0) {
      ...
      bool pok = (t > 0) ? trade.Buy (InpProbeLots, _Symbol, 0, 0.0, 0.0, "zbull_probe")
                         : trade.Sell(InpProbeLots, _Symbol, 0, 0.0, 0.0, "zbull_probe");
      if (pok) {
         ...
         PrintFormat("[BULL] [PROBE] scout %s @%.2f — %ds to prove %.2f pts",
   ...
   for (int q = 0; q < tickets; q++) {
      ...
      string tag = StringFormat("zbull_%s_D%d_m%d", (t > 0 ? "buy" : "sell"), dia, g_lawmask);
      ...
      if (pt > 0 && PositionSelectByTicket(pt)) {
         ...
         if (MathAbs(fill - px) > _Point) {
            ...
            if (!trade.PositionModify(pt, nsl, ntp))
               PrintFormat("[BULL] !! could not re-anchor #%I64u (%d) — still on QUOTE levels",
                           pt, trade.ResultRetcode());
            else if (MathAbs(fill - px) >= 1.0)
               PrintFormat("[BULL] !! fill %.2f vs quote %.2f (%.2f pts) — levels re-anchored "
   ...
   if (placed > 0) {
      g_last_fire = TimeCurrent();
      PrintFormat("[BULL] %s @%.2f — %d diamond(s) -> %d ticket(s), %.2f lots total · "
   ...
   for (int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if (TimeCurrent() - opened >= holdlim * 60)
         if (trade.PositionClose(t) && InpVerbose)
            PrintFormat("[BULL] aged out after %dm", InpMaxHoldMin);
   }
}
...
void FireBasket(int side, int dia, int mask, bool loud) {
   ...
   string tag = StringFormat("zbull_%s_D%d_m%d", (side > 0 ? "buy" : "sell"), dia, mask);
   ...
   if (placed > 0) {
      g_last_fire = TimeCurrent();
      PrintFormat("[BULL] BURST %s @%.2f — %d diamond(s) -> %d ticket(s), pre-tested ground",
...
void ProbeManage() {
   ...
   if (moved >= InpProbeMinPts) {
      PrintFormat("[BULL] [PROBE] conviction %.2f pts -> BURST", moved);
      FireBasket(g_probe_side, g_probe_dia, g_probe_mask, g_probe_loud);  // pre-tested environment
   } else if (InpVerbose)
      PrintFormat("[BULL] [PROBE] only %.2f pts -> retreat, no basket", moved);
}
