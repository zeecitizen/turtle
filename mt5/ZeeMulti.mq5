//+------------------------------------------------------------------+
//| ZeeMulti.mq5 — the other Feb-11 strategies, finally built.       |
//|                                                                  |
//| Zee, 2026-08-13: "let us implement them now, because only now for |
//| once our testing has reached a stage where things work."          |
//|                                                                  |
//| His Feb-11 day was FIVE patterns, reconstructed bar-by-bar from   |
//| the broker statement. We had mechanised one:                      |
//|                                                                  |
//|   1. UHV BREAKOUT      56%  inherited — this EA's core            |
//|   2. LIQUIDITY SWEEP   19%  NEW: wick beyond a recent extreme,    |
//|                             then CLOSE BACK INSIDE it             |
//|   3. NS/ND BREAK       11%  NEW: dead-volume narrow candle, then  |
//|                             broken in the trend direction         |
//|   4. MOMENTUM CONT.    41%  NEW: body/range >= 0.55 AND body      |
//|                             >= 1.5x the recent average body       |
//|   5. INTRABAND TAPE    33%  UN-MECHANISABLE and not attempted.    |
//|                             His own analysis: reading bid/ask     |
//|                             spikes inside the minute.             |
//|                                                                  |
//| The percentages overlap — one entry often carried two or three    |
//| patterns at once — so they do not sum to 100.                     |
//|                                                                  |
//| DELIBERATELY NOT BUILT: the "universal gate" of rng60_norm >=     |
//| 1.20 which the taxonomy calls necessary for every entry. It was   |
//| REFUTED on measurement — his entries median 1.04 against random   |
//| 1.02, and it would have blocked 9 of his 13 known trades. Reading  |
//| the taxonomy without that correction would have shipped it.       |
//|                                                                  |
//| Every pattern shares the SAME exit geometry and the same diamond  |
//| stack, so any difference in result is a difference in the ENTRY   |
//| and nothing else. Each is switchable and each is measured ALONE   |
//| before any combination is judged.                                 |
//|                                                                  |
//| magic 88101 — its own, so it can never manage another EA.         |
//+------------------------------------------------------------------+
...
input int    InpMagicNumber = 88101;  // InpMagicNumber — 88098 = ZeeUHV v1.2. Never 88094/88095/88096.
...
input group "-- The Feb-11 patterns (each measured ALONE first) --"
input bool   InpUseUhv      = true;   // InpUseUhv - 56% of his Feb-11 setups
input bool   InpUseSweep    = false;  // InpUseSweep - 19%: wick past an extreme, close back inside
input bool   InpUseNsnd     = false;  // InpUseNsnd - 11%: dead-volume narrow bar, then broken
input bool   InpUseMomentum = false;  // InpUseMomentum - 41%: big body in the trend direction
input int    InpSweepLook   = 20;     // InpSweepLook - bars defining "a recent extreme"
input double InpNsndVolMax  = 0.60;   // InpNsndVolMax - dead volume: below this fraction of the recent average
input double InpNsndRangeMax= 0.70;   // InpNsndRangeMax - and a narrow range, same basis
input double InpMomBody     = 0.55;   // InpMomBody - body/range floor (his taxonomy's number)
input double InpMomMult     = 1.5;    // InpMomMult - and body >= this x the recent average body
...
//+------------------------------------------------------------------+
//| PATTERN 2 - LIQUIDITY SWEEP + CLOSE BACK   (19% of Feb 11)       |
//|                                                                  |
//| His taxonomy: "recent extreme wicked BEYOND a key level, then     |
//| closed BACK INSIDE - stop-hunt rejection." The 17:37:26 buy came  |
//| after a low sweep, the 19:08:59 sell after a high sweep.          |
//|                                                                  |
//| To BUY: bar 1 dipped below the lowest low of the prior            |
//| InpSweepLook bars and then closed back above it. The stops        |
//| beneath were taken and price refused to stay down there.          |
//+------------------------------------------------------------------+
bool SweepSetup(int side) {
   if (side > 0) {
      double ext = bLow(2);
      for (int k = 2; k <= InpSweepLook + 1; k++) ext = MathMin(ext, bLow(k));
      return (bLow(1) < ext && bClose(1) > ext && IsGreen(1));
   }
   double ext2 = bHigh(2);
   for (int k = 2; k <= InpSweepLook + 1; k++) ext2 = MathMax(ext2, bHigh(k));
   return (bHigh(1) > ext2 && bClose(1) < ext2 && IsRed(1));
}

//+------------------------------------------------------------------+
//| PATTERN 3 - NO SUPPLY / NO DEMAND BREAK    (11% of Feb 11)       |
//|                                                                  |
//| "Dead-volume small candle (low body + low range), then BREAK of   |
//| its high/low in trend direction." His 01:59:04 and 02:29:02 buys. |
//|                                                                  |
//| This is Law 2 of his conviction laws, finally given its own entry |
//| instead of being a diamond. As a diamond it fired on 1.1% of      |
//| setups and could not affect anything; the classic No-Supply shape  |
//| does not sit where the UHV breakout looks. Here it is judged on    |
//| its own terms: bar 2 is the dead candle, bar 1 must break it.      |
//+------------------------------------------------------------------+
bool NsndSetup(int side) {
   double avgR = 0, avgV = 0;
   int n = 0;
   for (int k = 3; k <= 12; k++) {
      avgR += bHigh(k) - bLow(k);
      avgV += (double)BarVolume(k);
      n++;
   }
   if (n == 0) return false;
   avgR /= n;
   avgV /= n;
   if (avgR <= 0 || avgV <= 0) return false;
   double rng = bHigh(2) - bLow(2);
   if (rng > avgR * InpNsndRangeMax) return false;                 // narrow
   if ((double)BarVolume(2) > avgV * InpNsndVolMax) return false;  // and dead
   if (side > 0) return (bClose(1) > bHigh(2) && IsGreen(1));
   return (bClose(1) < bLow(2) && IsRed(1));
}

//+------------------------------------------------------------------+
//| PATTERN 4 - MOMENTUM CONTINUATION          (41% of Feb 11)       |
//|                                                                  |
//| "Big-body candle in trend direction: body/range >= 0.55 AND body  |
//| >= 1.5x recent average." Both thresholds are his taxonomy's own   |
//| numbers, not values I chose.                                      |
//+------------------------------------------------------------------+
bool MomentumSetup(int side) {
   double rng = bHigh(1) - bLow(1);
   if (rng <= 0) return false;
   double body = MathAbs(bClose(1) - bOpen(1));
   if (body / rng < InpMomBody) return false;
   double avg = 0;
   int n = 0;
   for (int k = 2; k <= 11; k++) { avg += MathAbs(bClose(k) - bOpen(k)); n++; }
   if (n == 0) return false;
   avg /= n;
   if (avg <= 0 || body < avg * InpMomMult) return false;
   return (side > 0) ? IsGreen(1) : IsRed(1);
}
...
//  both sides and take whichever completes a lawful setup.
   ...
   for (int si = 0; si < nsides; si++) {
      ...
      bool got = false;
      for (int rank = 0; InpUseUhv && rank < MathMax(1, InpUhvRank); rank++) {
      ...
      if (got) break;
      // THE OTHER FEB-11 PATTERNS. Each is a complete entry sharing the identical exit
      // and stack, so only the ENTRY differs. origin and uhv stay set for the diamond
      // count; a pattern with no UHV of its own scores from the retracement instead.
      if (InpUseSweep && SweepSetup(try_side)) {
         origin = o; uhv = MathMax(1, o / 2); side = try_side; got = true; break;
      }
      if (InpUseNsnd && NsndSetup(try_side)) {
         origin = o; uhv = 2; side = try_side; got = true; break;
      }
      if (InpUseMomentum && MomentumSetup(try_side)) {
         origin = o; uhv = MathMax(1, o / 2); side = try_side; got = true; break;
      }
   }
   if (side == 0) {
