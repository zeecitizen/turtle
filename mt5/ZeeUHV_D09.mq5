//+------------------------------------------------------------------+
//|  ZeeUHV_D09.mq5 — THE UNTOUCHED DIAMOND, resurrected         |
...
input int    InpMagicNumber = 88209;  // InpMagicNumber — 88094 = ZeeUHV, tester only
...
input double InpLawClosePos   = 0.0;   // >0: breakout must close within this share of its extreme
...
input double InpLawBreakEven  = 1.00;   // >0: stop to breakeven at this R
...
void LoadOandaVol() {
   ...
   if (h == INVALID_HANDLE) {
      Print("[D09] OANDA volume requested but oanda_vol.csv not found — using broker volume");
      return;
   }
   ...
   PrintFormat("[D09] OANDA volume table loaded: %d minutes (newest %s)",
...
int OnInit() {
   ...
   PrintFormat("[D09] ZeeUHV v1.10 — HIS rules from 146 labels. SL %.1f / TP %.1f · magic %d"
...
void OnDeinit(const int r) { PrintFormat("[D09] deinit reason=%d", r); }
...
void TryFire() {
   ...
   if (!WindowContinuous(InpTrendLook + 5)) {
      if (InpVerbose) Print("[D09] [SKIP] gap in lookback");
      return;
   }
...
//  both sides and take whichever completes a lawful setup.
   ...
   else { if (InpVerbose) Print("[D09] [SKIP] ranging — his setup needs a trend"); return; }
   ...
   if (side == 0) {
      if (InpVerbose && t != 0) Print("[D09] [SKIP] no lawful setup on the allowed side");
      return;
   }
   t = side;
   ...
   for (int q = 0; q < tickets; q++) {
      ...
      string tag = StringFormat("zd09_%s_D%d", (t > 0 ? "buy" : "sell"), dia);
   ...
   if (placed > 0) {
      ...
      PrintFormat("[D09] %s @%.2f — %d diamond(s) -> %d ticket(s), %.2f lots total · "
...
// LAWS.md line 43 — "after reaching 1:1 we make a BreakEven (we move the SL to
// breakeven)". The Diamond has never done this: its stop sits 20 points away and stays
// there for the whole 60 minutes. Once the trade has earned one unit of its own risk,
// this moves the stop to entry so the trade can no longer take money off the table.
// Runs on every tick, not on the bar, because a 1-point target is reached inside one.
void LawBreakEven() {
   if (InpLawBreakEven <= 0) return;
   for (int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if (risk <= 0) continue;
      if (isbuy  && sl >= entry) continue;            // already at or past breakeven
      if (!isbuy && sl <= entry && sl > 0) continue;
      double now = isbuy ? SymbolInfoDouble(_Symbol, SYMBOL_BID)
                         : SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      double gain = isbuy ? (now - entry) : (entry - now);
      if (gain < InpLawBreakEven * risk) continue;
      if (trade.PositionModify(tk, entry, tp) && InpVerbose)
         PrintFormat("[BE] #%I64u to breakeven at %.1fR", tk, InpLawBreakEven);
   }
}
   ...
   for (int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if (TimeCurrent() - opened >= InpMaxHoldMin * 60)
         if (trade.PositionClose(t) && InpVerbose)
            PrintFormat("[D09] aged out after %dm", InpMaxHoldMin);
   }
}
