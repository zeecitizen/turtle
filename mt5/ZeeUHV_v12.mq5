//+------------------------------------------------------------------+
//| ZeeUHV_v12.mq5 — v1.2. Two rules made switchable so they can be    |
//| MEASURED instead of assumed.                                      |
//|                                                                  |
//| Zee, 2026-08-13, on the local-peak rule:                          |
//|   "i meant all the neighbors in the retracement (of same colored  |
//|    candles)... and so its not BOTH / TWO of neighbors"           |
//|                                                                  |
//| He is right, and v1 does not implement what he specified. FindUhv |
//| already scans the retracement and keeps the LOUDEST SAME-COLOURED |
//| bar — that IS "largest among its peers". It then adds two extra   |
//| checks against the two ADJACENT bars, regardless of colour. So a  |
//| loud GREEN bar sitting beside a red UHV kills a buy setup, which  |
//| appears nowhere in his 146 labels. Measured on 1,920 minutes of   |
//| archived gold, those two lines alone reject 36% of all candidates.|
//|                                                                  |
//| His words on the body rule: "i think UHV should also be a strong  |
//| candle". That one IS his, so it is tested rather than removed.    |
//|                                                                  |
//| InpRequirePeak  false = his specification (loudest peer wins)     |
//|                 true  = v1's behaviour, kept so the difference    |
//|                         can be measured on real ticks             |
//|                                                                  |
//| magic 88098 — its own, so it can never manage v1's positions.     |
//+------------------------------------------------------------------+
...
input int    InpMagicNumber = 88098;  // InpMagicNumber — 88098 = ZeeUHV v1.2. Never 88094/88095/88096.
