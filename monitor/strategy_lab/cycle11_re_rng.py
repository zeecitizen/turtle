"""cycle11_re_rng.py — re-sweep rng_n_min and rng_min with current locked params.
Current: rng_n=1.5, rng=1.0. Maybe new optimal after all the other changes.
"""
...
print(f"  {'rng_n':>5} {'rng':>4} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for rng_n in [1.0, 1.2, 1.5, 1.8, 2.0, 2.5]:
    for rng in [0.5, 0.7, 1.0, 1.5, 2.0, 3.0]:
        ...
        print(f"  {rng_n:>5.1f} {rng:>4.1f} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6}", flush=True)
        if grand > best[0]: best = (grand, (rng_n, rng))

print(f"\n=== CYCLE 11 BEST: rng_n={best[1][0]} rng={best[1][1]} → ${best[0]*10:+.0f} ===", flush=True)
