"""cycle28_slippage_stress.py — CRITICAL robustness test.
Test current locked config at various COST levels to see if the edge holds under
realistic execution slippage.

COST = round-trip cost in raw price units. Current $0.20 assumes near-perfect fills.
Live broker reality:
  - $0.30 = optimistic (tight spreads, no slippage)
  - $0.50 = typical (small slippage on entry+exit)
  - $1.00 = conservative (larger slippage)
  - $2.00 = pessimistic (high-volatility execution)
"""
...
print(f"SLIPPAGE STRESS TEST — locked config at varying COST levels:")
print(f"  {'cost':>5} | {'fills':>5} {'WR%':>4} {'tot$':>8} {'Feb11$':>8} {'OOS$':>8} {'win_d':>5} {'loss_d':>6}", flush=True)
for cost in [0.20, 0.30, 0.50, 0.75, 1.00, 1.50, 2.00, 3.00]:
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, cost)
    ...
    print(f"  {cost:>5.2f} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+8.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+8.0f} {win_d:>5} {loss_d:>6}", flush=True)

print(f"\n=== CYCLE 28 DONE — slippage stress complete ===", flush=True)
