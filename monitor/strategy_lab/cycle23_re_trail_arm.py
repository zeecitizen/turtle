"""cycle23_re_trail_arm.py — re-sweep TRAIL_ARM with new locked params."""
...
TRAIL_GB = 10.0; MAX_LOSS = 10.0; SKIM = 10.0
...
def run_day(ticks, m5, trail_arm):
...
print(f"  {'arm':>5} | {'fills':>5} {'WR%':>4} {'tot$':>8} {'Feb11$':>8} {'OOS$':>8} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for arm in [0.2, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0]:
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, arm)
    ...
    print(f"  {arm:>5.1f} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+8.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+8.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, arm)

print(f"\n=== CYCLE 23 BEST: arm={best[1]} → ${best[0]*10:+.0f} ===", flush=True)
