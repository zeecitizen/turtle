"""cycle16_skim.py — sweep SKIM cap at new locked params."""
...
TRAIL_ARM = 2.0; TRAIL_GB = 10.0; MAX_LOSS = 10.0
...
print(f"  {'skim':>5} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for sk in [10.0, 15.0, 20.0, 30.0, 50.0, 75.0, 100.0, 999.0]:
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, sk)
    ...
    print(f"  {sk:>5.1f} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, sk)

print(f"\n=== CYCLE 16 BEST: skim={best[1]} → ${best[0]*10:+.0f} ===", flush=True)
