"""cycle6_cooldown.py — overnight cycle 6.
Sweep per-side cooldown. Current 60s. Try 20/30/45/90/120/180.
Tighter cooldown = more fills (good on trend days, bad on chop).
Wider cooldown = fewer fills (better selectivity).
"""
...
COST = 0.20
RNG_N_MIN = 1.5; RNG_MIN = 1.0; SPR_MAX = 0.40
...
print(f"  {'cd_sec':>6} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for cd in [20, 30, 45, 60, 90, 120, 180, 300]:
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, cd)
    ...
    print(f"  {cd:>6} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, cd)

print(f"\n=== CYCLE 6 BEST: cd={best[1]}s → ${best[0]*10:+.0f} ===", flush=True)
