"""cycle22_re_rng.py — re-sweep rng_n × rng_min with all new locked params."""
...
def run_day(ticks, m5, rng_n_min, rng_min):
...
print(f"  {'rng_n':>5} {'rng':>4} | {'fills':>5} {'WR%':>4} {'tot$':>8} {'Feb11$':>8} {'OOS$':>8} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for rng_n in [0.5, 0.8, 1.0, 1.2, 1.5]:
    for rng in [0.0, 0.3, 0.5, 0.8, 1.0]:
        feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
        for day, ticks, m5 in day_data:
            fills = run_day(ticks, m5, rng_n, rng)
        ...
        print(f"  {rng_n:>5.1f} {rng:>4.1f} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+8.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+8.0f} {win_d:>5} {loss_d:>6}", flush=True)
        if grand > best[0]: best = (grand, (rng_n, rng))

print(f"\n=== CYCLE 22 BEST: rng_n={best[1][0]} rng={best[1][1]} → ${best[0]*10:+.0f} ===", flush=True)
