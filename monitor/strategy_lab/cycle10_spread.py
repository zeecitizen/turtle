"""cycle10_spread.py — spread filter sweep."""
...
COST = 0.20
RNG_N_MIN = 1.5; RNG_MIN = 1.0; COOLDOWN_SEC = 45
...
print(f"  {'spr':>5} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for spr in [0.20, 0.25, 0.30, 0.35, 0.40, 0.50, 0.60, 1.00, 99.0]:
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, spr)
    ...
    print(f"  {spr:>5.2f} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, spr)

print(f"\n=== CYCLE 10 BEST: spr_max={best[1]} → ${best[0]*10:+.0f} ===", flush=True)
