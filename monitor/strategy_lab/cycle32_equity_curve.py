"""cycle32_equity_curve.py — cumulative P&L per day for visual proof.
Both intra-day cumulative (within Feb 11) and day-over-day cumulative across 23 days.
Output: ASCII equity curve + table.
"""
...
for path in day_files:
    ...
    day_pnl = sum(f["pnl"] for f in fills) * 10  # dollars at 0.10 lots
    day_results.append((day_str, day_pnl, len(fills)))

# Cumulative
print(f"=== DAY-BY-DAY EQUITY CURVE (cumulative $ at 0.10 lots, COST=$0.50) ===\n")
print(f"  {'date':<12}{'day$':>9}{'cum$':>9}  bar (1 char = $5000)")
cum = 0
max_dd = 0
peak_cum = 0
for day_str, pnl, n in day_results:
    cum += pnl
    if cum > peak_cum: peak_cum = cum
    dd = peak_cum - cum
    if dd > max_dd: max_dd = dd
    bar_len = max(0, int(abs(cum) / 5000))
    if cum >= 0:
        bar = "█" * min(80, bar_len)
        sign = "+"
    else:
        bar = "▒" * min(80, bar_len)
        sign = "-"
    print(f"  {day_str:<12}{pnl:>+9.0f}{cum:>+9.0f}  {bar}")

print(f"\n=== STATS ===")
print(f"  Final equity:    ${cum:+.0f}")
print(f"  Peak equity:     ${peak_cum:+.0f}")
print(f"  Max drawdown:    ${max_dd:.0f} (from peak)")
print(f"  Days traded:     {len(day_results)}")
print(f"  Days positive:   {sum(1 for _, p, _ in day_results if p > 0)}")
print(f"  Days negative:   {sum(1 for _, p, _ in day_results if p < 0)}")
print(f"  Total fills:     {sum(n for _, _, n in day_results):,}")
print(f"  Best day:        ${max(p for _, p, _ in day_results):+.0f}")
print(f"  Worst day:       ${min(p for _, p, _ in day_results):+.0f}")

print(f"\n=== CYCLE 32 EQUITY CURVE DONE ===")
