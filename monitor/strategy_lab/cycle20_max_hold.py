"""cycle20_max_hold.py — sweep MAX_HOLD_SEC (timeout exit)."""
...
DAILY_DD_STOP = 75.0; M5_LB = 14; CHECK_EVERY = 20
...
def run_day(ticks, m5, max_hold_sec):
...
print(f"  {'maxhold':>7} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for mh in [120, 300, 600, 900, 1200, 1800, 2400, 3600]:
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, mh)
    ...
    print(f"  {mh:>7} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, mh)

print(f"\n=== CYCLE 20 BEST: max_hold={best[1]}s → ${best[0]*10:+.0f} ===", flush=True)
