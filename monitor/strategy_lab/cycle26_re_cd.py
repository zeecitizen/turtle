"""cycle26_re_cd.py — re-sweep cooldown one more time at final-ish params."""
...
print(f"  {'cd_sec':>6} | {'fills':>5} {'WR%':>4} {'tot$':>8} {'Feb11$':>8} {'OOS$':>8} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for cd in [1, 3, 5, 10, 15, 20, 30, 45]:
    ...
    print(f"  {cd:>6} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+8.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+8.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, cd)

print(f"\n=== CYCLE 26 BEST: cd={best[1]}s → ${best[0]*10:+.0f} ===", flush=True)
