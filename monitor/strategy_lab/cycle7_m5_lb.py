"""cycle7_m5_lb.py — overnight cycle 7. Sweep M5_LB (trend lookback bars).
20 bars = 100 min (short trend). 30 bars = 150 min (current). 40 = 200 min, 50 = 250 min.
"""
...
DAILY_DD_STOP = 35.0; CHECK_EVERY = 20
MAX_HOLD_SEC = 1800
...
print(f"  {'m5_lb':>5} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for lb in [10, 14, 20, 25, 30, 40, 50, 60]:
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, lb)
    ...
    print(f"  {lb:>5} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, lb)

print(f"\n=== CYCLE 7 BEST: m5_lb={best[1]} → ${best[0]*10:+.0f} ===", flush=True)
