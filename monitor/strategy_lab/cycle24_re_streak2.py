"""cycle24_re_streak2.py — re-sweep loss-streak again with all new locked params."""
...
DAILY_DD_STOP = 100.0; M5_LB = 14; CHECK_EVERY = 20
MAX_HOLD_SEC = 2400
TRAIL_ARM = 5.0; TRAIL_GB = 10.0; MAX_LOSS = 10.0; SKIM = 10.0
...
print(f"  {'N':>3} {'pause':>5} | {'fills':>5} {'WR%':>4} {'tot$':>8} {'Feb11$':>8} {'OOS$':>8} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
configs = [(0, 0)]
for n in [1, 2, 3, 4]:
    for p in [60, 180, 300, 600, 900, 1200, 1800, 2700, 3600]:
        configs.append((n, p))
for cfg in configs:
    n, p = cfg
    ...
    print(f"  {n:>3} {p:>5} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+8.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+8.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, cfg)

print(f"\n=== CYCLE 24 BEST: N={best[1][0]} pause={best[1][1]}s → ${best[0]*10:+.0f} ===", flush=True)
