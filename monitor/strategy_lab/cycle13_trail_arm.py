"""cycle13_trail_arm.py — sweep TRAIL_ARM (trail activation threshold)."""
...
TRAIL_GB = 5.0; MAX_LOSS = 5.0; SKIM = 50.0
...
print(f"  {'arm':>4} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for arm in [0.2, 0.5, 0.7, 1.0, 1.5, 2.0, 3.0]:
    ...
    print(f"  {arm:>4.1f} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, arm)

print(f"\n=== CYCLE 13 BEST: arm={best[1]} → ${best[0]*10:+.0f} ===", flush=True)
