"""cycle9_dd.py — sweep daily DD threshold with new locked params."""
...
print(f"  {'DD':>4} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6} {'worst':>6}", flush=True)
best = (-99999, None)
for dd in [20, 25, 30, 35, 40, 50, 75, 100, 999]:
    ...
    label = "off" if dd >= 999 else f"{dd}"
    print(f"  {label:>4} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6} {worst_day*10:>+6.0f}", flush=True)
    if grand > best[0]: best = (grand, dd)

print(f"\n=== CYCLE 9 BEST: DD={best[1]} → ${best[0]*10:+.0f} ===", flush=True)
