"""cycle8_streak.py — overnight cycle 8.
Re-sweep loss-streak params now that M5_LB=20 (was 30) and cd=45 (was 60).
"""
...
COST = 0.20
RNG_N_MIN = 1.5; RNG_MIN = 1.0; SPR_MAX = 0.40; COOLDOWN_SEC = 45
DAILY_DD_STOP = 35.0; M5_LB = 20; CHECK_EVERY = 20
MAX_HOLD_SEC = 1800
...
def run_day(ticks, m5, streak_n, pause_sec):
    ...
    for k in range(50, len(ticks), CHECK_EVERY):
        ...
        if pnl > 0: consec_losses = 0
            ...
            consec_losses += 1
            if streak_n > 0 and consec_losses >= streak_n:
...
print(f"  {'N':>3} {'pause':>5} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
configs = [(0, 0)]  # no streak (baseline of no-streak with new params)
for n in [2, 3, 4, 5, 6]:
    for p in [300, 600, 900, 1200, 1500, 1800]:
        configs.append((n, p))
for cfg in configs:
    n, p = cfg
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, n, p)
        if not fills: continue
        nn = len(fills); w = sum(1 for f in fills if f["pnl"]>0)
        tot = sum(f["pnl"] for f in fills)
        tot_fills += nn; tot_wins += w
    ...
    print(f"  {n:>3} {p:>5} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, cfg)

print(f"\n=== CYCLE 8 BEST: N={best[1][0]} pause={best[1][1]}s → ${best[0]*10:+.0f} ===", flush=True)
