"""cycle25_re_check.py — re-sweep CHECK_EVERY with all new locked params."""
...
def run_day(ticks, m5, check_every):
...
print(f"  {'every':>5} | {'fills':>5} {'WR%':>4} {'tot$':>8} {'Feb11$':>8} {'OOS$':>8} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for ce in [3, 5, 10, 15, 20, 30, 50, 100]:
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, ce)
    ...
    print(f"  {ce:>5} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+8.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+8.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, ce)

print(f"\n=== CYCLE 25 BEST: every={best[1]} → ${best[0]*10:+.0f} ===", flush=True)
