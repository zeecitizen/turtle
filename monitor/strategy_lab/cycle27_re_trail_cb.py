"""cycle27_re_trail_cb.py — 2D re-sweep of trail_gb × max_loss × skim at final params."""
...
def run_day(ticks, m5, trail_gb, max_loss, skim):
...
print(f"  {'gb':>4} {'CB':>4} {'skim':>5} | {'fills':>5} {'WR%':>4} {'tot$':>8} {'Feb11$':>8} {'OOS$':>8} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for gb in [5.0, 7.0, 10.0, 15.0]:
    for cb in [5.0, 7.0, 10.0, 15.0, 20.0]:
        for skim in [5.0, 10.0, 15.0, 25.0, 50.0]:
            feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
            for day, ticks, m5 in day_data:
                fills = run_day(ticks, m5, gb, cb, skim)
            ...
            print(f"  {gb:>4.1f} {cb:>4.1f} {skim:>5.1f} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+8.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+8.0f} {win_d:>5} {loss_d:>6}", flush=True)
            if grand > best[0]: best = (grand, (gb, cb, skim))

print(f"\n=== CYCLE 27 BEST: gb={best[1][0]} CB={best[1][1]} skim={best[1][2]} → ${best[0]*10:+.0f} ===", flush=True)
