"""cycle2_skip_open.py — overnight cycle 2.
Hypothesis: first 10-15 min of each session have transition chop. Skip them.
"""
...
COST = 0.20
RNG_N_MIN = 1.5; RNG_MIN = 1.5; SPR_MAX = 0.40; COOLDOWN_SEC = 60
...
# Variant: skip_open_min = number of minutes to skip at the START of each session
SKIP_VARIANTS = [0, 5, 10, 15, 20, 30]


def session_active(t, skip_min):
    mins = t.hour * 60 + t.minute
    s1_start = 90 + skip_min;  s1_end = 150
    s2_start = 1005 + skip_min; s2_end = 1185
    if s1_start <= mins <= s1_end: return True
    if s2_start <= mins <= s2_end: return True
    return False
...
def run_day(ticks, m5, skip_open_min):
    ...
    for k in range(50, len(ticks), CHECK_EVERY):
        ...
        if not session_active(dt, skip_open_min): continue
...
print(f"  {'skip_min':>8} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
for skip_min in SKIP_VARIANTS:
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_days = 0; loss_days = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, skip_min)
    ...
    print(f"  {skip_min:>8} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_days:>5} {loss_days:>6}", flush=True)

print(f"\n=== CYCLE 2 DONE ===", flush=True)
