"""zee_feb11_ea_v7.py — v1's detector + MT5-style intrabar path simulation.

MT5 "M1 OHLC" backtest model order:
  Green bar (c > o): open → high → low → close
  Red bar   (c < o): open → low → high → close

This MUCH better reflects real momentum: on a strong green continuation bar, price
goes UP first (capture peak), THEN retraces. Bar-conservative "low before high" was
artificially whipsawing winners.

Test on Feb 11 with this corrected intra-bar order. Expect v1-level fires (loose
detector) with much higher avgW.
"""
import csv, sys
...
def walk_bar_mt5(bar, entry_px, side, peak, armed, trail_arm, trail_gb, skim, max_loss):
    """Walk one bar in MT5 control-point order, returning (exit_pnl, exit_reason, new_peak, new_armed)
    or (None, ...) if no exit in this bar.

    Order:
      Green bar: open → high → low → close
      Red bar:   open → low → high → close
      Doji (c == o): treat as low → high (worst case for trail, best for CB)
    """
    if bar["c"] > bar["o"]:
    ...
    for px in path:
        if side == "buy":
            ...
            cur = entry_px - px
        if cur >= skim:
            return (skim - COST, "SKIM", peak, armed)
        peak = max(peak, cur)
        if peak >= trail_arm: armed = True
        if armed and cur <= peak - trail_gb:
            return (cur - COST, "TRAIL", peak, armed)
        if cur <= -max_loss:
            return (-max_loss - COST, "CB", peak, armed)
    return (None, None, peak, armed)
...
def simulate(entry_idx, side, entry_px):
    ...
    for j in range(entry_idx, min(entry_idx + MAX_HOLD, N)):
        bar = bars[j]
        exit_pnl, why, peak, armed = walk_bar_mt5(
            bar, entry_px, side, peak, armed, TRAIL_ARM, TRAIL_GIVEBACK, SKIM_CAP, MAX_LOSS
        )
        if exit_pnl is not None:
            return (exit_pnl, j, why)
        held = j - entry_idx + 1
        cur_close = (bar["c"] - entry_px) if side == "buy" else (entry_px - bar["c"])
        if held >= SCRATCH_BARS and peak < TRAIL_ARM and cur_close <= SCRATCH_AT:
            return (cur_close - COST, j, "SCRATCH")
...
for i in range(N):
    ...
    for side in ("buy", "sell"):
        ...
        fills.append({"t": bars[i]["t"], "side": side, "entry": entry_px, "pnl": pnl,
                      "why": why, "patterns": ",".join(sorted(h.keys()))})
...
print(f"=== FEB 11 v7 RESULTS (MT5-OHLC intrabar order) ===")
