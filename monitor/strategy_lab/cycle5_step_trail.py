"""cycle5_step_trail.py — overnight cycle 5.
STEP TRAIL: as peak grows, raise the trail floor proportionally to lock in profit.

Modes:
  - "fixed" (baseline): trail = peak - gb_fixed
  - "pct": trail = peak * pct  (e.g. 0.5 = give back 50% of peak)
  - "ratchet": at peak thresholds {3, 5, 10}, floor moves to {1, 3, 6}
  - "atr": gb = atr_mult * M5_atr  (adaptive to volatility)
"""
...
TRAIL_ARM = 1.0; MAX_LOSS = 5.0; SKIM = 50.0
...
def trail_floor(peak, mode, gb_fixed, pct, ratchet, atr):
    """Returns the minimum acceptable exit level (cur >= floor means we still hold)."""
    if mode == "fixed":
        return peak - gb_fixed
    if mode == "pct":
        return peak * pct
    if mode == "ratchet":
        # ratchet is list of (peak_threshold, lock_floor)
        floor = peak - gb_fixed
        for thr, lock in ratchet:
            if peak >= thr:
...
def run_day(ticks, m5, mode, gb_fixed, pct, ratchet):
    ...
    for k in range(50, len(ticks), CHECK_EVERY):
        ...
        for j in range(k, len(ticks)):
            ...
            if armed:
                floor = trail_floor(peak, mode, gb_fixed, pct, ratchet, 0)
                if cur <= floor: exit_pnl = cur; break
...
configs = [
    ("fixed",   5.0, None, None),                        # baseline
    ("pct",     None, 0.3, None),                        # give back 70%
    ("pct",     None, 0.4, None),                        # give back 60%
    ("pct",     None, 0.5, None),                        # give back 50%
    ("pct",     None, 0.6, None),                        # give back 40%
    ("ratchet", 5.0, None, [(3.0, 1.0), (8.0, 4.0)]),    # lock-in at $3 → floor $1, at $8 → floor $4
    ("ratchet", 5.0, None, [(2.0, 0.5), (5.0, 2.0), (10.0, 5.0)]),
    ("ratchet", 3.0, None, [(2.0, 0.5), (5.0, 2.0)]),    # tighter base + ratchet
    ("ratchet", 7.0, None, [(5.0, 2.0), (10.0, 6.0)]),   # wider base + ratchet
]
print(f"  {'mode':<8} {'gb':>4} {'pct':>4} {'ratchet':<30} | {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for cfg in configs:
    mode, gb, pct, ratchet = cfg
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, mode, gb if gb else 5.0, pct if pct else 0.5, ratchet if ratchet else [])
    ...
    rstr = str(ratchet) if ratchet else "-"
    print(f"  {mode:<8} {str(gb):>4} {str(pct):>4} {rstr:<30} | {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, cfg)

print(f"\n=== CYCLE 5 BEST: {best[1]} → ${best[0]*10:+.0f} ===", flush=True)
