"""cycle21_re_dd.py — re-sweep DD with current locked params (cd=15, M5_LB=14)."""
...
SPR_MAX = 0.50; COOLDOWN_SEC = 15
M5_LB = 14; CHECK_EVERY = 20
MAX_HOLD_SEC = 2400
LOSS_STREAK_N = 2; LOSS_STREAK_PAUSE = 600
TRAIL_ARM = 2.0; TRAIL_GB = 10.0; MAX_LOSS = 10.0; SKIM = 10.0
...
def run_day(ticks, m5, dd_stop):
    ...
    for k in range(50, len(ticks), CHECK_EVERY):
        ...
        if dd_stop > 0 and daily_pnl <= -dd_stop: continue
...
print(f"  {'DD':>5} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6} {'worst':>6}", flush=True)
best = (-99999, None)
for dd in [20, 35, 50, 75, 100, 150, 200, 999]:
    feb_pnl = 0; oos_pnl = 0; tot_fills = 0; tot_wins = 0; win_d = 0; loss_d = 0; worst = 0
    for day, ticks, m5 in day_data:
        fills = run_day(ticks, m5, dd)
    ...
    print(f"  {dd:>5} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6} {worst*10:>+6.0f}", flush=True)
    if grand > best[0]: best = (grand, dd)

print(f"\n=== CYCLE 21 BEST: DD={best[1]} → ${best[0]*10:+.0f} ===", flush=True)
