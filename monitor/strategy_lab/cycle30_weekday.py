"""cycle30_weekday.py — weekday + intraday breakdown to find any structural bias."""
...
for path in day_files:
    ...
    day_data.append((day_str, sum(f["pnl"] for f in fills), len(fills),
                     datetime.strptime(day_str, "%Y-%m-%d").weekday(), fills))
    all_fills.extend(fills)

print(f"STABILITY: weekday + session breakdown (COST=$0.50)\n", flush=True)

# === Weekday breakdown ===
print(f"=== Per-weekday performance ===")
wd_data = {}
for day_str, pnl, n, wd, _ in day_data:
    wd_data.setdefault(wd, []).append((day_str, pnl, n))
print(f"  {'wd':<5}{'days':>5}{'avg$':>10}{'total$':>10}{'WR%':>6}")
for wd in range(7):
    if wd not in wd_data: continue
    rows = wd_data[wd]
    avg = statistics.mean(p for _, p, _ in rows)
    tot = sum(p for _, p, _ in rows)
    wins = sum(1 for _, p, _ in rows if p > 0)
    print(f"  {WEEKDAYS[wd]:<5}{len(rows):>5}{avg*10:>+10.0f}{tot*10:>+10.0f}{100*wins/len(rows):>5.0f}%")

# === Session breakdown ===
print(f"\n=== Session (morning 01:30-02:30 vs evening 16:45-19:45) ===")
morning_fills = [f for f in all_fills if in_morning(f["t"])]
evening_fills = [f for f in all_fills if in_evening(f["t"])]
print(f"  Morning: n={len(morning_fills)}, "
      f"NET=${sum(f['pnl'] for f in morning_fills):+.0f} raw "
      f"= ${sum(f['pnl'] for f in morning_fills)*10:+.0f} at 0.10L, "
      f"WR={100*sum(1 for f in morning_fills if f['pnl']>0)/max(1,len(morning_fills)):.0f}%")
print(f"  Evening: n={len(evening_fills)}, "
      f"NET=${sum(f['pnl'] for f in evening_fills):+.0f} raw "
      f"= ${sum(f['pnl'] for f in evening_fills)*10:+.0f} at 0.10L, "
      f"WR={100*sum(1 for f in evening_fills if f['pnl']>0)/max(1,len(evening_fills)):.0f}%")

# === Side breakdown ===
print(f"\n=== Side breakdown ===")
buys = [f for f in all_fills if f["side"] == "buy"]
sells = [f for f in all_fills if f["side"] == "sell"]
for label, sub in [("BUY", buys), ("SELL", sells)]:
    if not sub: continue
    n = len(sub); w = sum(1 for f in sub if f["pnl"]>0)
    tot = sum(f["pnl"] for f in sub)
    print(f"  {label}: n={n}, WR={100*w/n:.0f}%, NET=${tot:+.0f} raw = ${tot*10:+.0f} at 0.10L")

# === Single-day variance/distribution ===
print(f"\n=== Day P&L distribution ===")
pnls = [d[1] for d in day_data]
pnls.sort()
for p_label, idx in [("min", 0), ("25%", len(pnls)//4), ("50%", len(pnls)//2),
                      ("75%", 3*len(pnls)//4), ("max", len(pnls)-1)]:
    print(f"  {p_label:<5}: ${pnls[idx]:+.0f} raw = ${pnls[idx]*10:+.0f}")

print(f"\n=== CYCLE 30 WEEKDAY DONE ===", flush=True)
