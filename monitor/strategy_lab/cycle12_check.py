"""cycle12_check.py — sweep CHECK_EVERY tick interval."""
...
print(f"  {'every':>5} | {'fills':>5} {'WR%':>4} {'tot$':>7} {'Feb11$':>8} {'OOS$':>7} {'win_d':>5} {'loss_d':>6}", flush=True)
best = (-99999, None)
for ce in [5, 10, 15, 20, 30, 50, 100]:
    ...
    print(f"  {ce:>5} | {tot_fills:>5} {wr:>3.0f}% {grand*10:>+7.0f} {feb_pnl*10:>+8.0f} {oos_pnl*10:>+7.0f} {win_d:>5} {loss_d:>6}", flush=True)
    if grand > best[0]: best = (grand, ce)

print(f"\n=== CYCLE 12 BEST: every={best[1]} → ${best[0]*10:+.0f} ===", flush=True)
