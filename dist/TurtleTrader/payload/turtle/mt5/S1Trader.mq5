...
//── Inputs ──────────────────────────────────────────────────────────
input group "── Sizing ──"
input double InpLots          = 0.30;     // 2026-06-16 v2.76: REVERTED 0.80→0.30 after live receipt showed 0.80 + tight SL cap = -$1668 across 99 trades. At 0.30 lots: worst-case 6pt UHV blow-up = ~$144 (survivable). Wide SL lets winners ride to TP. Slower but real.
input int    InpMagicNumber   = 88005;    // 2026-06-09 v2.64: defaults to M1 scalp instance (was 88004 = M5). M5 instance running v2.63 stays untouched.
input double InpDailyLossHalt = 200.0;  // 2026-06-16 v2.76: scaled to 0.30 lot size (500 × 0.30/0.80 ≈ 188 → round to 200). One uncapped 6pt stop ≈ $144; halt at $200 = 1.4 such killers max. ~10% of demo acct. 0=off.
...
input double InpBreakoutMinBodyPts      = 1.5;   // minimum body size (in price units) of the forming bar at trigger time. For SELL: open-close >= this. For BUY: close-open >= this.
...
input double InpMinSweepDepthPts       = 0.30; // 2026-06-22 v2.97: minimum depth (pts) the sweep must reach past UHV extreme. Master: "sweep must be real, not a tiny tap." 0=off.
input double InpMaxBreakoutWickPct     = 0.0;  // 2026-06-22 v2.97: max against-direction wick on breakout candle (as fraction of range). Master: "weak candle = long wick against direction." 0=off. Recommended 0.35 once enabled.
...
input double InpMinBreakoutPenetration = 0.30;  // 2026-06-08 v2.62: minimum price-units the forming bar's body must close BEYOND the UHV extreme. Blocks "micro-poke" breakouts (Zee m34/m35). For BUY: ask must >= uhv_h + this. For SELL: bid must <= uhv_l - this.
...
input group "── Exit ──"
input double InpTPPoints          = 1.3;  // 2026-06-22 v2.98: 5.0→1.3 — with rejection-wick filter producing 87.5% WR at TP=1.3pt, return to "catch the fish" tight TP. Master prefers high WR over high $.
