"""build_setups4_m1.py — render the M1 tuned fires with binary verdict labelling UI.

Uses monitor/strategy_lab/_canonical/results_m1_7d.json from soft-vrat-only config:
body=0.50, mom=0.65, vrat=0.85, sl=1.5, tp=3.0, hard_trend=True.

Output: monitor/setup_labels/setups4.html (served at setups.claudezeeshan.com/setups4.html).
Labels save with key prefix 'n' (e.g. n1, n2, …) so they don't collide with m1-m48.
"""
...
RESULTS = REPO / "monitor" / "strategy_lab" / "_canonical" / "results_m1_7d.json"
OUT = REPO / "monitor" / "setup_labels"
OUT.mkdir(parents=True, exist_ok=True)


def build_m1_bars(date_csvs: list[Path], t_start: datetime, t_end: datetime) -> pd.DataFrame | None:
    rows = []
    for p in date_csvs:
        try:
    ...
    bars: dict = {}
    for dt, mid in rows:
        bt = dt.replace(second=0, microsecond=0)  # M1 buckets
        b = bars.get(bt)
...
def render(idx: int, fire: dict, png_path: Path) -> bool:
    ...
    t_start = open_t - timedelta(minutes=60)
    t_end   = open_t + timedelta(minutes=45)
    date_csvs = []
    ...
    df = build_m1_bars(date_csvs, t_start, t_end)
    ...
    for i, t in enumerate(df.index):
        if abs((t - origin_t).total_seconds()) < 30:
            origin_m[i] = df["Low"].iloc[i] - rng*0.07
        if abs((t - uhv_t).total_seconds()) < 30:
            uhv_m[i] = df["High"].iloc[i] + rng*0.05
        if abs((t - brk_t).total_seconds()) < 30:
    ...
    title = (f"M1 Setup #{idx}  |  {side}  |  {outcome} {pnl_str}  |  "
...
def main():
    ...
    fires = d["fires"]
    print(f"Loaded {len(fires)} M1 fires (config: soft-vrat-only, last 7 days).")
    cards = []
    for i, fire in enumerate(fires, 1):
        png = OUT / f"setup4_{i:03d}.png"
    ...
    for i, f in cards:
        ...
        <div class="card" id="card-{i}">
          <div class="hdr">
            <h2>M1 Setup #{i} — {f['side']} · {f['open_t']}</h2>
            <span class="badge" style="background:{outcome_color}">{f['outcome']} {pnl_s}</span>
          </div>
          <img src="setup4_{i:03d}.png" loading="lazy"/>
          <div class="quiz">
            ...
            <div class="reason-row" id="reason-row-{i}" style="display:none;">
              ...
              <textarea id="reason-{i}" data-idx="{i}" placeholder="Y/B/R shorthand, or what gate failed…"></textarea>
            </div>
...
<title>M1 Tuned Setups — verify before scaling</title>
<style>
  body {{ font-family: -apple-system, system-ui, sans-serif; background: #0b0e14; color: #e6edf3; margin: 0; padding: 16px; }}
  h1 {{ font-size: 22px; margin: 6px 0; }}
  .intro {{ background: #161b22; padding: 14px; border-radius: 10px; margin-bottom: 18px;
    font-size: 13px; line-height: 1.5; border-left: 4px solid #58a6ff; }}
  ...
  .verdict-opt {{ flex: 1; display: flex; align-items: center; padding: 12px 14px;
    ...
    cursor: pointer; font-size: 15px; font-weight: 600; gap: 8px; transition: all 0.15s; }}
  .verdict-opt:hover {{ background: #1f242c; }}
...
<h1>M1 Tuned Setups — verify before scaling ({len(cards)} setups)</h1>
<div class="progress"><span id="progress-text">0 / {len(cards)} labelled</span><div class="bar"><div class="fill" id="progress-fill"></div></div></div>
<div class="intro">
  <b>What this is:</b> The M1 instance you just attached (magic 88005, SL 1.5/TP 3.0) would have fired these
  {len(cards)} trades in the LAST 7 DAYS of XAUUSD ticks. Backtest result: <b>{d['wins']}W / {d['losses']}L = {d['wr_pct']:.1f}% WR, +{d['pnl_pts']:.2f} pts</b>.
  <br/>If you mark most as correct, the M1 config is safe to keep live. If many are wrong, tell me what gate's off and
  I'll tune the M1 EA accordingly.<br/><br/>
  <b>Markers:</b> 🟦 cyan ■ = retracement origin · 🔴 red ▼ = UHV (BUY) · 🟢 green ▼ = UHV (SELL) ·
  🔵 blue ▲ = BUY breakout · 🔴 red ▼ above = SELL breakout · white dashed = entry · red dotted = SL · green dotted = TP.
</div>
...
async function save(idx) {{
  ...
  if (!verdict) {{
    document.getElementById('status-' + idx).textContent = 'pick Correct or Incorrect first';
    return;
  }}
  const payload = {{ idx: 'n' + idx, who: 'zee', label: JSON.stringify({{ verdict, reason }}) }};
...
fetch('/api/labels').then(r => r.json()).then(d => {{
  ...
  updateProgress();
    ...
    (OUT / "setups4.html").write_text(html, encoding="utf-8")
    print(f"\nWrote {OUT/'setups4.html'} ({len(cards)} cards)")
    print(f"Public URL: https://setups.claudezeeshan.com/setups4.html")
