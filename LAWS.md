...
UHV
an ultra high volume candle occurs within a valid retracement. so in a retracement (a pullback) we compare all red colored candle's volumes. the largest red colored volume (we read volume from tradingview's OANDA volume chart), this largest volume candle is the one we call the ultra high volume candle. once found we mark its high and its low. the high price, we wait for a breakout candle to break that level and close above this high. 
...
we only trade in the NewYork session (US session) that starts at Pakistani 6:00 PM, because the price moves fast then
