...
| 12 | 2026-06-04 ~02:00 PKT (Claude via TASK-005 + Gemini DR) | Even the MQL5-mirror's +$312/27d at 0.05L was overstated — broker SL=$25 was cutting losses tighter than EA's internal CB=$50 | 2026-06-04 ~02:00 PKT | **REFUTED partial: "realistic" mirror still depended on broker parachute.** Built  per Gemini DR — pure state-machine, AGGRESSIVE config, NO broker SL/TP. Result: -$45.66/27d at 0.05L. **Profit Factor 0.99** (Gemini's criterion: ≤1.0 = edge was fake). WR 48.5%. The +$477k AGGRESSIVE / +$167k MEDIUM / +$312 mirror — all variants of the same chronological-leak family. The broker SL=$25 in mirror was acting as a tighter stop than the EA's CB=$50, so the mirror's "profit" was the broker auto-protecting capital, NOT a strategy edge. Real path to TASK-001 north star: find filter combinations with realistic PF ≥ 1.25, NOT lot scaling on a PF=0.99 strategy. |
