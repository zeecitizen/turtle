...
|---|---|---|---|
| v1.53 | 2026-08-20 | PULSE-SWITCHED TARGET (`InpRedTargetPts`, default off): red pulse → TP 0.75 (the chop harvester — near-misses at 0.7-0.9 pts convert to wins), green/diamond → TP 1.0 (feast pays full) | born from the +605.28 autopsy: TP 0.75's virgin gains live in chop halves (Feb2 WR 69→82%), its live-day losses in feasts (Aug 11 −66 while WR rose). Triple exam running — ships only if positive on all three datasets |
